...
fn rsa_perf() {
    ...
    for keypair in keys {
        ...
        let min_speed = speeds.iter().min().unwrap()/m as u128;

        println!("PKCS1: {} ns", (min_speed).to_string());
    }
    //PSS
    ...
    for keypair in keys {
        ...
        let result = min_speed / m as u128;

        println!("PSS: {} ns", result.to_string());
    }
}
...
fn ec_perf() {
    ...
    let min_speed = speeds.iter().min().unwrap()/m as u128;
    println!("NIST P-256: {} ns", min_speed.to_string());
    ...
    println!("NIST P-384: {} ns", min_speed.to_string());
}
